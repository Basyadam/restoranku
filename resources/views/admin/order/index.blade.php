...
@section('page-subtitle', 'Daftar pesanan pelanggan')

@section('content')
<div class="row mb-4">
    <div class="col-12">
        <div class="rounded bg-light p-4">
            <h5 class="mb-3"><i class="fa fa-clipboard-list me-2 text-primary"></i>Daftar Pesanan</h5>
            ...
            @if($orders->isEmpty())
                <div class="text-center py-5">
                    <i class="fa fa-cart-xmark fa-3x text-muted mb-3"></i>
                            ...
                            <td>
                                <div class="d-flex gap-1 flex-wrap">
                                    <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="d-inline">
                                        ...
                                        <button type="submit" class="btn btn-success btn-sm" {{ $order->status == 'settlement' || $order->status == 'cooked' ? 'disabled' : '' }}>
                                            <i class="fa fa-check-circle"></i> Settle
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        ...
                                        <button type="submit" class="btn btn-primary btn-sm" {{ $order->status == 'cooked' ? 'disabled' : '' }}>
                                            <i class="fa fa-fire"></i> Masak
                                        </button>
                                    </form>
                                    ...
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <div class="row mb-3">
                                                    <div class="col-md-6">
                                                        <p class="mb-1"><strong>Kode Pesanan:</strong> {{ $order->order_ccode }}</p>
                                                        <p class="mb-1"><strong>Tanggal:</strong> {{ $order->created_at->format('d/m/Y H:i') }}</p>
                                                        <p class="mb-1"><strong>Metode Bayar:</strong> {{ $order->payment_method == 'qris' ? 'QRIS' : 'Tunai' }}</p>
                                                    </div>
                                                    <div class="col-md-6">
